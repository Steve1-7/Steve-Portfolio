  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      ...
      <nav className={`fixed top-0 w-full z-50 border-b transition-all duration-500 ${
        scrolled ? 'bg-slate-900/95 backdrop-blur-xl border-slate-700 shadow-2xl shadow-cyan-500/10' : 'bg-slate-900/80 backdrop-blur-md border-slate-800'
      }`}>
        <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3 group cursor-pointer">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center text-xl font-bold border-2 border-cyan-400 group-hover:scale-110 group-hover:rotate-12 transition-all duration-300 shadow-lg shadow-cyan-500/50">SR</div>
            <span className="text-lg font-semibold group-hover:text-cyan-400 transition-colors">Steve Ronald</span>
          </div>
          <div className="hidden md:flex gap-6">
            {["About", "Projects", "Skills", "Contact"].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setActiveSection(item.toLowerCase())} className={`relative hover:text-cyan-400 transition-all duration-300 ${activeSection === item.toLowerCase() ? "text-cyan-400" : "text-slate-300"} group`}>
                {item}
                <span className={`absolute -bottom-1 left-0 h-0.5 bg-gradient-to-r from-cyan-400 to-blue-500 transition-all duration-300 ${activeSection === item.toLowerCase() ? 'w-full' : 'w-0 group-hover:w-full'}`} />
              </a>
            ))}
          </div>
          <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden flex flex-col justify-center items-center w-10 h-10 gap-1.5 rounded-lg border border-slate-700 hover:border-cyan-400/50 transition-all hover:scale-110">
        ...
        <div className={`md:hidden transition-all duration-300 overflow-hidden ${menuOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="px-6 pb-6 pt-3 flex flex-col gap-2 border-t border-slate-800 bg-slate-900/98">
            {[
              ...
              <a key={item.label} href={item.href} onClick={() => { setActiveSection(item.label.toLowerCase()); setMenuOpen(false); }} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeSection === item.label.toLowerCase() ? "bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 scale-105" : "text-slate-300 hover:bg-slate-800/80 hover:text-cyan-400 border border-transparent hover:scale-105"}`}>
                <span className="text-xl">{item.icon}</span>
                <span className="font-medium">{item.label}</span>
                {activeSection === item.label.toLowerCase() && <span className="ml-auto w-2 h-2 bg-cyan-400 rounded-full animate-pulse" />}
              </a>
            ))}
            <div className="flex gap-3 mt-3 pt-4 border-t border-slate-800">
              <a href="https://github.com/Steve1-7" target="_blank" rel="noopener noreferrer" className="flex-1 text-center py-2.5 rounded-xl border border-slate-700 text-slate-400 hover:border-cyan-400/50 hover:text-cyan-400 text-sm font-medium transition-all hover:scale-105">GitHub</a>
              <a href="https://www.linkedin.com/in/steve-ronald1710s/" target="_blank" rel="noopener noreferrer" className="flex-1 text-center py-2.5 rounded-xl border border-slate-700 text-slate-400 hover:border-cyan-400/50 hover:text-cyan-400 text-sm font-medium transition-all hover:scale-105">LinkedIn</a>
              <a href="mailto:stevezuluu@gmail.com" className="flex-1 text-center py-2.5 rounded-xl border border-slate-700 text-slate-400 hover:border-cyan-400/50 hover:text-cyan-400 text-sm font-medium transition-all hover:scale-105">Email</a>
            </div>
          </div>
        </div>
      </nav>
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="flex flex-col items-center text-center">
            <div className="relative flex items-center justify-center mb-16">
              ...
              <div className="absolute w-56 h-56 rounded-full bg-cyan-500/10 blur-md animate-pulse" />
              <div className="relative w-52 h-52 rounded-full overflow-hidden border-4 border-cyan-400 shadow-[0_0_40px_rgba(56,189,248,0.5)] z-10 group hover:scale-110 transition-transform duration-300">
                <img src="https://i.ibb.co/5gRLQG7C/Whats-App-Image-2025-01-08-at-10-16-10-0b7c5513.jpg" alt="Steve Ronald" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
              </div>
              <div className="absolute -bottom-4 z-20 bg-slate-900 border border-slate-700 rounded-full px-4 py-1.5 flex items-center gap-2 shadow-lg hover:scale-110 transition-transform cursor-pointer">
                <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse" />
                <span className="text-xs text-slate-300 font-medium">Available for work</span>
              </div>
            </div>
            <h1 className="text-6xl md:text-7xl font-bold mb-4 bg-gradient-to-r from-white via-cyan-200 to-blue-400 bg-clip-text text-transparent">Steve Ronald</h1>
            ...
            <div className="flex gap-4 mb-8">
              <a href="#contact" className="px-8 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-lg font-semibold hover:shadow-lg hover:shadow-cyan-500/50 transition-all hover:scale-110 hover:-translate-y-1">Get In Touch</a>
              <a href="#projects" className="px-8 py-3 border border-slate-600 rounded-lg font-semibold hover:border-cyan-400 hover:text-cyan-400 transition-all hover:scale-110 hover:-translate-y-1">View Work</a>
            </div>
            <div className="flex gap-6">
              <a href="https://github.com/Steve1-7" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-cyan-400 transition-all text-sm hover:scale-125">GitHub</a>
              <a href="https://www.linkedin.com/in/steve-ronald1710s/" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-cyan-400 transition-all text-sm hover:scale-125">LinkedIn</a>
              <a href="mailto:stevezuluu@gmail.com" className="text-slate-400 hover:text-cyan-400 transition-all text-sm hover:scale-125">Email</a>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center mb-24">
            <div className="flex flex-col items-center">
              <div className="relative flex items-center justify-center mb-16">
                ...
                <div className="relative w-52 h-52 rounded-full overflow-hidden border-4 border-cyan-400 shadow-[0_0_40px_rgba(56,189,248,0.5)] z-10 group hover:scale-110 transition-transform duration-300">
                  <img src="https://i.ibb.co/1GVHNhmj/hyb.jpg" alt="Steve Ronald" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
                </div>
              ...
              <div className="grid grid-cols-3 gap-4 w-full mt-4">
                <div className="bg-slate-900/80 border border-slate-700 rounded-2xl p-5 text-center hover:border-cyan-500/50 hover:scale-105 transition-all cursor-pointer">
                  <p className="text-3xl font-bold text-cyan-400 mb-1">3+</p>
                  <p className="text-xs text-slate-400 leading-tight">Years Experience</p>
                </div>
                <div className="bg-slate-900/80 border border-slate-700 rounded-2xl p-5 text-center hover:border-purple-500/50 hover:scale-105 transition-all cursor-pointer">
                ...
                <div className="bg-slate-900/80 border border-slate-700 rounded-2xl p-5 text-center hover:border-orange-500/50 hover:scale-105 transition-all cursor-pointer">
            ...
            <div>
              ...
              <div className="flex gap-4 mt-8">
                <a href="mailto:stevezuluu@gmail.com" className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-lg font-semibold text-sm hover:shadow-lg hover:shadow-cyan-500/30 transition-all hover:scale-105">Hire Me</a>
                <a href="#projects" className="px-6 py-3 border border-slate-600 rounded-lg font-semibold text-sm hover:border-cyan-400 hover:text-cyan-400 transition-all hover:scale-105">See My Work</a>
              </div>
            </div>
          </div>
      ...
      <section id="projects" className="py-28 px-6 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            {/* King's Barbershop */}
            <div className="group relative bg-slate-800/60 rounded-3xl border border-slate-700 overflow-hidden hover:border-amber-500/50 transition-all duration-500 hover:scale-[1.02]">
            ...
            {/* Saseka Woodworks */}
            <div className="group relative bg-slate-800/60 rounded-3xl border border-slate-700 overflow-hidden hover:border-emerald-500/50 transition-all duration-500 hover:scale-[1.02]">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            {[
              ...
              <div key={project.title} className={`group relative bg-slate-800/60 rounded-2xl border border-slate-700 overflow-hidden ${project.border} transition-all duration-500 hover:scale-105`}>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="group relative bg-slate-800/60 rounded-2xl border border-slate-700 overflow-hidden hover:border-blue-500/50 transition-all duration-500 flex hover:scale-[1.02]">
            ...
            <div className="group relative bg-slate-800/60 rounded-2xl border border-slate-700 overflow-hidden hover:border-yellow-500/50 transition-all duration-500 flex hover:scale-[1.02]">
          ...
          <div className="mt-16 text-center">
            ...
            <a href="https://github.com/Steve1-7" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-8 py-4 border border-slate-600 rounded-xl font-semibold hover:border-cyan-400 hover:text-cyan-400 transition-all hover:scale-105">View All on GitHub →</a>
          </div>
        </div>
      </section>
      ...
      <section id="skills" className="py-28 px-6 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            {/* Frontend */}
            <div className="group relative bg-slate-900/60 rounded-3xl border border-slate-700 hover:border-cyan-500/50 transition-all duration-500 overflow-hidden p-8 hover:scale-[1.02]">
              <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <div className="relative z-10">
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-cyan-400 to-blue-500 flex items-center justify-center text-2xl shadow-lg shadow-cyan-500/20 group-hover:scale-110 transition-transform">🖥️</div>
                  <div>
            ...
            {/* Backend */}
            <div className="group relative bg-slate-900/60 rounded-3xl border border-slate-700 hover:border-emerald-500/50 transition-all duration-500 overflow-hidden p-8 hover:scale-[1.02]">
              <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <div className="relative z-10">
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-2xl shadow-lg shadow-emerald-500/20 group-hover:scale-110 transition-transform">⚙️</div>
                  <div>
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-8">
            {[
              ...
              <div key={group.title} className={`group relative bg-slate-900/60 rounded-3xl border border-slate-700 ${group.border} transition-all duration-500 overflow-hidden p-8 hover:scale-105`}>
                <div className="relative z-10">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {group.tools.map(tool => (
                      <div key={tool.name} className={`flex items-center gap-2 bg-slate-800/60 border border-slate-700 rounded-xl px-3 py-2.5 ${group.toolBorder} transition-all duration-300 hover:scale-105 cursor-pointer`}>
          ...
          <div className="bg-slate-900/60 border border-slate-700 rounded-3xl p-8 hover:scale-[1.01] transition-transform">
      ...
      <section id="contact" className="py-28 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <a href="mailto:stevezuluu@gmail.com" className="inline-block px-10 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-lg font-semibold text-lg hover:shadow-lg hover:shadow-cyan-500/50 transition-all mb-16 hover:scale-110 hover:-translate-y-1">Send Message</a>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={contact.label} className={`bg-slate-800/50 p-6 rounded-xl border border-slate-700 ${contact.color} transition-all hover:scale-105 cursor-pointer`}>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 p-3 bg-cyan-500/90 text-white rounded-full shadow-lg hover:bg-cyan-400 transition-all hover:scale-110 ${scrolled ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}
      ...
      <div className="relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {timelineData.map((item, index) => (
            <div key={index} className="group relative cursor-pointer transition-all duration-500 hover:scale-105" onClick={() => setActiveIndex(activeIndex === index ? null : index)} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)}>
  ...
  return (
    <div>
      ...
      <div className="flex justify-center gap-3 mb-10 flex-wrap">
        {filters.map(f => (
          <button key={f} onClick={() => { setFilter(f); setActiveCard(null); }} className={`px-6 py-2.5 rounded-full text-sm font-semibold border transition-all duration-300 hover:scale-105 ${filter === f ? "bg-gradient-to-r from-cyan-500 to-blue-600 border-transparent text-white shadow-lg shadow-cyan-500/25" : "bg-slate-900/60 border-slate-700 text-slate-400 hover:border-cyan-500/50 hover:text-cyan-400"}`}>
            {f === "All" && "✦ "}{f === "Development" && "⚙️ "}{f === "Design" && "🎨 "}{f === "Marketing" && "📣 "}{f}
            <span className="ml-2 text-xs opacity-60">({f === "All" ? strengths.length : strengths.filter(s => s.category === f).length})</span>
          </button>
        ))}
      </div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((item, index) => (
          <div key={index} onClick={() => setActiveCard(activeCard === index ? null : index)} className={`group relative bg-slate-900/60 rounded-2xl border border-slate-700 ${item.border} p-7 transition-all duration-500 overflow-hidden cursor-pointer hover:scale-105 ${activeCard === index ? "ring-1 ring-cyan-400/40 scale-105" : ""}`}>
      ...
      <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-slate-900/60 border border-slate-700 rounded-2xl p-5 text-center hover:border-slate-600 transition-all hover:scale-105 cursor-pointer">
      ...
      <div className="mt-10 bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-purple-500/10 border border-slate-700 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 hover:scale-[1.02] transition-transform">
        <div>
        ...
        <a href="mailto:stevezuluu@gmail.com" className="shrink-0 px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl font-semibold hover:shadow-lg hover:shadow-cyan-500/30 transition-all whitespace-nowrap hover:scale-110">Let's Talk →</a>
      </div>
    </div>
  );
}
